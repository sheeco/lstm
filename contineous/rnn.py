...
class TheanoRNN:
    ...
    def __init__(self, location_dim = 2, hidden_him = 100, bptt_truncate = 4):
        ...
        U = np.random.uniform(-np.sqrt(1.0/location_dim), np.sqrt(1.0/location_dim), (hidden_him, location_dim))
        V = np.random.uniform(-np.sqrt(1.0/hidden_him), np.sqrt(1.0/hidden_him), (location_dim, hidden_him))
        ...
        # self.U = theano.shared(name = 'U', value = U.astype('float32'))
        # self.V = theano.shared(name = 'V', value = V.astype('float32'))
        # self.W = theano.shared(name = 'W', value = W.astype('float32'))
    ...
    def __theano_build__(self):
        ...
        # 表达式 x为输入 y输出 均为float的矩阵
        # 输入类似于[[x11, x12], [x21, x22]....[xn1, xn2]]
        # x, y为1个训练样例!!!
        x = T.matrix('x', theano.config.floatX)
        y = T.vector('y', theano.config.floatX)
        ...
        # 计算预测值
        self.forward_propagation = function([x], o)
    ...
    def caculate_loss(self, X, Y):
        ...
        return max_loss, min_loss, 1.0 * avg_loss / num
